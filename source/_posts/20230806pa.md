...
Passkey 的完整流程需要前后端配合，核心是以下 4 个接口：
...
- **注册**：`startRegistration(result)` 调用浏览器原生 WebAuthn API，弹出系统级指纹/人脸/PIN 验证；成功后把 `attResp`（包含公钥凭证）发给后端验证并保存
- **登录**：`startAuthentication(result)` 同样调用 WebAuthn 完成设备验证，后端校验签名后返回 `token`，前端拿到后即可视为登录成功
- 出现 `InvalidStateError` 表示该设备上的凭据已注册过，需提示用户
