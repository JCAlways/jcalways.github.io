...
categories:
  - OnlyOffice
keywords: OnlyOffice
description: OnlyOffice 前端使用教程：通过 JavaScript API 在网页中集成在线文档编辑器
cover: https://static-www.onlyoffice.com/v9.5.0/images/press-downloads/logos/logo_color.svg
---

## 前端使用

OnlyOffice 文档服务器（Document Server）提供了前端 SDK，通过简单的 `DocsAPI.DocEditor` 即可在网页中嵌入在线文档的查看与编辑功能。整体流程分三步：

1. **引入 SDK**：在页面中加载文档服务器提供的 `api.js`（注意域名需替换为你的 OnlyOffice 服务地址）
2. **配置 config 对象**：声明文档信息（`document`）、编辑配置（`editorConfig`）与事件回调（`events`）
3. **实例化编辑器**：`new DocsAPI.DocEditor(容器id, config)` 将编辑器挂载到指定 DOM 中
...
<html lang="en">
  ...
  <body>
    <script
      ...
      src="https://example.com/web-apps/apps/api/documents/api.js"
    ></script>
    <div id="preview"></div>
    <script type="module">
      let config = {
...
| 字段 | 说明 |
| --- | --- |
| `documentType` | 文档类型：`text`（文档）/ `spreadsheet`（表格）/ `presentation`（演示文稿） |
| `key` | 文档唯一标识，每次编辑保存后**必须重新生成**，否则会提示版本过期 |
| `title` | 文件名，下载文档时也会使用该名称 |
| `url` | 存储原始文档的绝对 URL，OnlyOffice 服务会从这里拉取文件 |
| `permissions` | 权限配置：`edit`（可否编辑）、`print`（可否打印）、`download`（可否下载）等 |

### editorConfig（编辑配置）

- `mode`：`view`（只读视图）或 `edit`（编辑模式）
- `callbackUrl`：**保存文件时的回调地址**，用户点保存后 OnlyOffice 服务会把编辑后的文件回传给该地址，由后端接收并落库
- `lang`：界面语言，如 `zh-CN`
- `customization`：界面定制（是否显示帮助、聊天、自动保存、徽标 logo 等）
- `user`：当前操作者信息（`id`、`name`），用于协同编辑时标识用户
...
- `onDocumentStateChange`：文档内容被修改（`data` 为 `true` 表示有未保存修改）
- `onError`：发生错误
- `onRequestSaveAs`：用户点击"另存为"

```js
// 监听文档修改状态，用于提示用户是否保存
docEditor.events.on('documentStateChange', function (e) {
  console.log(e.data); // true = 有未保存的修改
});
```

> 注意：`key` 在每次保存后必须重新生成，否则再次打开会触发 `onOutdatedVersion` 错误提示，需用新的 `key` 重新初始化编辑器。
